...
def image_recover(x, diff):
    ...
    if B == 0:
        # 返回一个形状合适的零张量
        return torch.zeros(1, C, H, W + diff)
...
def test(args):
    ...
    for items in test_dataloader:
        ...
        # image = image_recover(image,diff)
        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            if args.adapter:
                for i in range(args.epoch):
                    ...
                    for layer in range(len(patch_tokens_ln)):
                        ...
                        # 检查张量大小并安全地分割
                        if anomaly_map.size(0) >= 2:
                            anomaly_map_n = image_recover(anomaly_map[:2], diff)
                        else:
                            # 如果不足2个元素，用零张量代替
                            shape = anomaly_map.shape
                            anomaly_map_n = torch.zeros(1, shape[1], shape[2], shape[3] + diff)
                        ...
                        if anomaly_map.size(0) > 2:
                            ...
                            # 如果不足2个元素后的切片，用零张量代替
                            shape = anomaly_map.shape
                            anomaly_map_a = torch.zeros(1, shape[1], shape[2], shape[3] + diff)
            ...
            for layer in range(len(patch_tokens_ln)):
                ...
                anomaly_map = torch.nn.functional.avg_pool2d(anomaly_map, 3, stride=1, padding=0,
                                                             count_include_pad=False)
                anomaly_map = F.interpolate(anomaly_map,
                                            size=img_size, mode='bilinear', align_corners=True)
                # 确保anomaly_map不是空张量
                if anomaly_map.size(0) > 0:
                    anomaly_map = image_recover(anomaly_map, diff)
                else:
                    # 创建一个形状合适的零张量作为替代
                    anomaly_map = torch.zeros(1, anomaly_map.size(1), img_size, img_size + diff)
            ...
            vis = cv2.cvtColor(cv2.resize(cv2.imread(path[0]), (anomaly_map.shape[-1], anomaly_map.shape[-2])),
